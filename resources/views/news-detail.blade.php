...
@section('content')
    ...
    <div class="relative flex flex-col gap-4 pt-40 md:gap-10 mb-16 md:mb-8 px-4 lg:px-8">
            ...
            <div class="relative w-full min-h-[300px] h-[300px] md:h-[500px] rounded-lg overflow-hidden group"
                ...
                @if(count($heroImages) > 1)
                    ...
                    <div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                        @foreach($heroImages as $index => $img)
                            <button data-index="{{ $index }}"
                                class="slider-dot w-2.5 h-2.5 rounded-full bg-white/50 hover:bg-white transition-all {{ $index === 0 ? 'bg-white scale-125' : '' }}"
                    ...
                    <button id="prev-slide-btn"
                        class="absolute left-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white p-2 rounded-full backdrop-blur-sm transition opacity-0 group-hover:opacity-100">
                    ...
                    <button id="next-slide-btn"
                        class="absolute right-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white p-2 rounded-full backdrop-blur-sm transition opacity-0 group-hover:opacity-100">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                            stroke="currentColor" class="w-6 h-6">
        ...
        <!-- Article Body -->
        <div class="prose max-w-none md:max-w-[1020px] text-left mx-auto">
            {!! $news->content !!}
        </div>
